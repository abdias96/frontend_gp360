<template>
  <div>
    ...
    <div class="card mb-5">
      <div class="card-body py-4">
        <div class="d-flex align-items-center">
          <i class="ki-duotone ki-file-sheet fs-2x text-primary me-4">
            <span class="path1"></span>
            <span class="path2"></span>
          </i>
          <div>
            <h2 class="mb-1">{{ $t('documents.legalDocuments.title') }}</h2>
            <p class="text-muted mb-0">{{ $t('documents.legalDocuments.subtitle') }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Upload Modal (with legal/judicial types only) -->
    <DocumentUploadModal
      :show="showUploadModal"
      :allowed-file-types="['legal', 'judicial']"
      default-file-type="legal"
...
// Legal document types filter (predefined)
const LEGAL_FILE_TYPES = ['legal', 'judicial'] as const;
...
// Load files with legal/judicial type filter
const loadFiles = async () => {
  ...
  try {
    const params: FileQueryParams = {
      ...
      file_type: LEGAL_FILE_TYPES as unknown as FileQueryParams['file_type'],
      ...filters
    };
