<template>
  <ModuleListTemplate
    :title="t('rehabilitation.education.basic.title')"
    :subtitle="t('rehabilitation.education.basic.subtitle')"
    :items="items"
    :loading="loading"
    :total="total"
    ...
    :empty-state-text="t('rehabilitation.education.basic.noData')"
    @search="handleSearch"
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">{{ editingItem ? t('rehabilitation.actions.edit') : t('rehabilitation.actions.create') }} - {{ t('rehabilitation.education.basic.title') }}</h5>
...
const PROGRAM_TYPE_FILTER = 'basic_education';
...
const resolveProgramType = async () => {
  try {
    ...
    const found = types.find((pt: any) =>
      pt.name?.toLowerCase().includes('basica') || pt.name?.toLowerCase().includes('basic') || pt.code?.toLowerCase() === 'basic_education'
    );
...
const loadItems = async () => {
  ...
  try {
    ...
    console.error('Error loading basic education programs:', error);
