<template>
  <ModuleListTemplate
    :title="t('rehabilitation.education.literacy.title')"
    :subtitle="t('rehabilitation.education.literacy.subtitle')"
    :items="items"
    :loading="loading"
    :total="total"
    ...
    :empty-state-text="t('rehabilitation.education.literacy.noData')"
    @search="handleSearch"
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">{{ editingItem ? t('rehabilitation.actions.edit') : t('rehabilitation.actions.create') }} - {{ t('rehabilitation.education.literacy.title') }}</h5>
...
const PROGRAM_TYPE_FILTER = 'literacy';
...
const resolveProgramType = async () => {
  try {
    ...
    const found = types.find((pt: any) =>
      pt.name?.toLowerCase().includes('alfabet') || pt.name?.toLowerCase().includes('literacy') || pt.code?.toLowerCase() === 'literacy'
    );
...
const loadItems = async () => {
  ...
  try {
    ...
    console.error('Error loading literacy programs:', error);
  ...
  Swal.fire({
    ...
    html: `
      <div class="text-start">
        ...
        <p><strong>${t('rehabilitation.programs.columns.duration')}:</strong> ${item.duration_weeks || '-'}</p>
