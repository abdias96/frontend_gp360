...
const modalFields: ModalField[] = [
  ...
  },
  {
    ...
    type: "text",
    required: true,
    placeholder: "Código único de la ocupación",
  },
  {
    key: "description",
    label: "Descripción",
...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query("/catalogs/occupations", params);
...
const createItem = async (formData: any) => {
  try {
    ...
    const response = await ApiService.post("/catalogs/occupations", formData);
...
const deleteItem = async (id: number) => {
  try {
    const response = await ApiService.delete(`/catalogs/occupations/${id}`);
