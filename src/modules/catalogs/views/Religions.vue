...
const modalFields: ModalField[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Ingrese el nombre de la religión",
  },
  {
    key: "code",
    label: "Código",
    type: "text",
    required: true,
    placeholder: "Ingrese el código de la religión",
  },
  {
    key: "description",
    label: "Descripción",
...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query("/catalogs/religions", params);
...
const createItem = async (formData: any) => {
  try {
    ...
    const response = await ApiService.post("/catalogs/religions", formData);
...
const deleteItem = async (id: number) => {
  try {
    const response = await ApiService.delete(`/catalogs/religions/${id}`);
