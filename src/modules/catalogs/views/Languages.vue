...
const modalFields: ModalField[] = [
  ...
  },
  {
    ...
    type: "text",
    required: true,
    placeholder: "Código único del idioma",
  },
  {
    key: "description",
    label: "Descripción",
...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query("/catalogs/languages", params);
...
const createItem = async (formData: any) => {
  try {
    ...
    const response = await ApiService.post("/catalogs/languages", formData);
...
const deleteItem = async (id: number) => {
  try {
    const response = await ApiService.delete(`/catalogs/languages/${id}`);
