...
const modalFields: ModalField[] = [
  ...
  },
  {
    ...
    type: "text",
    required: true,
    placeholder: "Código único de la nacionalidad",
  },
  {
    key: "description",
    label: "Descripción",
    type: "textarea",
    required: false,
    placeholder: "Descripción de la nacionalidad",
  },
  {
    key: "active",
    label: "Activo",
...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query("/catalogs/nationalities", params);
...
const createItem = async (formData: any) => {
  try {
    ...
    const response = await ApiService.post("/catalogs/nationalities", formData);
...
const deleteItem = async (id: number) => {
  try {
    const response = await ApiService.delete(`/catalogs/nationalities/${id}`);
