<template>
  <div>
    ...
    <CatalogList
      catalog-name="Tipo de Documento"
      :items="items"
      :loading="loading"
...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query("/catalogs/document-types", params);
...
const deleteItem = async (id: number) => {
  try {
    const response = await ApiService.delete(`/catalogs/document-types/${id}`);
