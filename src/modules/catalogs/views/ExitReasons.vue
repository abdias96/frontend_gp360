...
const modalFields: ModalField[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Ingrese el nombre del motivo de egreso",
  },
  {
    key: "code",
    label: "Código",
  ...
  },
  {
    ...
    type: "textarea",
    required: false,
    placeholder: "Descripción del motivo de egreso",
  },
  {
    key: "active",
    label: "Activo",
...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query("/catalogs/exit-reasons", params);
...
const createItem = async (formData: any) => {
  try {
    ...
    const response = await ApiService.post("/catalogs/exit-reasons", formData);
...
const editItem = async (id: number, formData: any) => {
  try {
    ...
    console.error("Error updating exit reason:", error);
  }
};

const deleteItem = async (id: number) => {
  try {
    const response = await ApiService.delete(`/catalogs/exit-reasons/${id}`);
