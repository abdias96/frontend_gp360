...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query("/catalogs/disabilities", params);
...
const createItem = async (formData: any) => {
  try {
    ...
    const response = await ApiService.post("/catalogs/disabilities", formData);
...
const deleteItem = async (id: number) => {
  try {
    const response = await ApiService.delete(`/catalogs/disabilities/${id}`);
