<template>
  <div>
    ...
    <CatalogList
      catalog-name="Sanción Disciplinaria"
      :items="items"
      :loading="loading"
...
const modalFields: ModalField[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Ingrese el nombre de la sanción disciplinaria",
  },
  {
    key: "code",
    label: "Código",
  ...
  },
  {
    ...
    type: "textarea",
    required: false,
    placeholder: "Descripción de la sanción disciplinaria",
  },
  {
    key: "active",
    label: "Activo",
...
const loadItems = async (page = 1, search = "") => {
  ...
  try {
    ...
    const response = await ApiService.query(
      "/catalogs/disciplinary-sanctions",
      params,
    );
    ...
    console.error("Error loading disciplinary sanctions:", error);
  } finally {
    loading.value = false;
  }
};
...
const editItem = async (id: number, formData: any) => {
  try {
    ...
    const response = await ApiService.put(
      `/catalogs/disciplinary-sanctions/${id}`,
      formData,
    );
